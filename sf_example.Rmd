...
Let's get started!  We'll start with some simple density calculations that are a version of something I use all the time.  But first, let's get into R.  It's pretty simple.  We'll just install and run it locally.  You just need to download [R](https://cran.r-project.org/) and [RStudio](https://rstudio.com/products/rstudio/download/).  After that open up RStudio.  Go to File > New Project > New Directory > New Project and create a new project.  Then go to File > New File > R Script.  You should be looking at something like this.
...
Let's get started with some analysis.  First, we'll download a free shapefile to work with.  In this case, the well spots and lines from the COGCC.
